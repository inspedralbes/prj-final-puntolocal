...
async function getComercioData() {
    ...
    if (response) {
        if (response.imagenes) {
            try {
                const parsed = JSON.parse(response.imagenes);
                response.imagenes = parsed.map(img => `http://localhost:8000/storage/${img}`);
            } catch (error) {
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div><label
                                    ...
                                    Medio</label><input type="number" name="puntaje_medio" min="0" max="5" step="0.1"
