<template>
    ...
    <div v-else>
        <nav class="sticky top-[76px] z-50 flex gap-4 w-full justify-center bg-gray-200 text-lg">
            <button class="border p-2 hover:bg-gray-500 rounded-lg"
                @click="showSection('productos'); getProductsBySearch()">Productes</button>
            <button class="hover:bg-gray-500 rounded-lg"
                @click="showSection('comercios'); getComerciosBySearch()">Comerços</button>
        </nav>
        <div v-if="currentSection === 'productos' && Object.values(productos).length > 0"
